...
export const ClientStories = () => {
  ...
  useEffect(() => {
    axios
      .get<Testimonial[]>("http://localhost:3000/api/testimonials")
      .then((res) => setStories(res.data))
      .catch((err) => console.error("Error fetching stories:", err));
  }, []);

  // Static icon image URL
  const staticIcon =
    "https://codia-f2c.s3.us-west-1.amazonaws.com/image/2025-08-18/Us7cP2PJ8d.png";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {stories.map((story) => (
            <div
              key={story._id}
              className="bg-[#f2efe9] border border-[#c6c6c6] p-4 sm:p-6"
            >
              <div
                className="w-8 h-8 bg-cover bg-no-repeat mb-3"
                style={{ backgroundImage: `url(${staticIcon})` }}
              />
              <blockquote
                className="text-base sm:text-lg lg:text-xl font-semibold text-[#746b5f] leading-relaxed mb-6"
                style={{ fontFamily: "Cormorant Garamond, serif" }}
